"""
Hierarchical Tier-1+2 MCMC - Full 7 Clusters
=============================================

Re-train hierarchical model including A1689 and MACS1149 to remove
sampling bias. Excludes only MACS0717 (major merger).

This addresses systematic underprediction on high-mass hold-outs.
...
OUTPUT_DIR = Path(__file__).parent.parent / 'output' / 'hierarchical_tier12_full7'
Q_PLANE_GRID = np.linspace(0.6, 1.4, 9)
Q_LOS_GRID = np.linspace(0.6, 1.4, 9)

# ALL 7 RELAXED CLUSTERS (excluding only MACS0717 merger)
TRAIN_CLUSTERS = ['MACS0416', 'A2744', 'A370', 'RXJ1347', 'CL0024', 'A1689', 'MACS1149']
...
if __name__ == '__main__':
    print("="*70)
    print("HIERARCHICAL TIER-1+2 MCMC - FULL 7 CLUSTERS")
    print("="*70)
    
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    print(f"\\nConfiguration:")
    print(f"  Sample: 7 relaxed clusters (MACS0717 excluded)")
    ...
    for j in range(N_WALKERS):
        pos[j, 0] = np.clip(pos[j, 0], 12.0, 22.0)
        pos[j, 1] = np.clip(pos[j, 1], 0.5, 5.0)
        for i in range(len(cluster_names_train)):
            pos[j, 2+4*i] = np.clip(pos[j, 2+4*i], 12.0, 22.0)
    ...
    _cosmo = cosmo
    _cluster_names = cluster_names_train
    ...
    with Pool(N_PROCESSES, initializer=_init_worker, 
              initargs=(baryon_cache, cosmo, cluster_names_train)) as pool:
    ...
    for i, cluster_name in enumerate(cluster_names_train):
        ...
        print(f"                   A_c={A_c:.2f}, M_500={cache['M_500']/1e15:.2f}e15")
    ...
    with open(OUTPUT_DIR / 'summary.txt', 'w', encoding='utf-8') as f:
        f.write(f"Full 7-Cluster Hierarchical Calibration\\n")
        f.write(f"="*70 + "\\n\\n")
    ...
    # Plots
    fig = corner.corner(flat_samples[:, :2], labels=[r'$\mu_A$', r'$\sigma_A$'],
    ...
    err = [baryon_cache[r['cluster']]['theta_E_err'] for r in train_results]
    ...
    print("\\n" + "="*70)
    print("FULL 7-CLUSTER CALIBRATION COMPLETE")
    print("="*70)
