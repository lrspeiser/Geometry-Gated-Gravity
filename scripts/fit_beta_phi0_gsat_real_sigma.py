...
OUTCSV = ROOT / 'data' / 'clash' / 'processed' / 'einstein_radii_baryon_beta_phi0_gsat.csv'
METRICS = ROOT / 'data' / 'clash' / 'processed' / 'eval' / 'er_baryon_beta_phi0_gsat_metrics.json'
BEST = ROOT / 'data' / 'clash' / 'processed' / 'eval' / 'best_beta_phi0_gsat.json'
...
def main():
    ...
    phi0_grid = np.array([1e6, 3e6, 1e7, 3e7, 1e8], dtype=float)
    gsat_grid = np.array([1.0e3, 3.0e3, 1.0e4, 3.0e4], dtype=float)

    best = None

    def predict_for(cid: str, beta: float, phi0: float, gsat: float) -> float | None:
        local = namemap[cid]
        zl = zmap[cid]
    ...
    for gsat in gsat_grid:
        for p0 in phi0_grid:
            for b in betas:
                preds = []
                targs = []
                for cid in train_ids:
                    p = predict_for(cid, b, p0, gsat)
                ...
                if (best is None) or (score < best['mae_arcsec']):
                    best = {'beta': float(b), 'phi0_km2s2': float(p0), 'g_sat': float(gsat), 'mae_arcsec': float(score), 'count': len(preds)}
                    print(f"[grid] new best beta={b:.3f} phi0={p0:.3e} g_sat={gsat:.1f} train_MAE={score:.3f} (n={len(preds)})")

    if best is None:
        print('No valid beta/phi0/gsat fit (no predictions).')
        return

    # Predict for all with best hyperparams
    rows = []
    for cid, _, _ in CLASH:
        p = predict_for(cid, best['beta'], best['phi0_km2s2'], best['g_sat'])
    ...
    print('[beta-phi0-gsat-fit] best:', best)
    print('[beta-phi0-gsat-fit] metrics:', json.dumps(results, indent=2))
