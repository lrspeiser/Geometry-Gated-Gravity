...
- Rotmod LTG Parquet (rows across many galaxies)
  Default: /Users/leonardspeiser/Projects/GravityCalculator/sparc_rotmod_ltg.parquet
...
Power-law G(M_bary)
  G_pred = A * (M_bary / 1e10 Msun)^beta
  Default A and beta derived from your earlier fit (excluding ultra-diffuse):
    A = 5.589712133866334
    beta = -0.6913962885091143

Usage
  python scripts/sparc_predict.py \
    --parquet /Users/leonardspeiser/Projects/GravityCalculator/sparc_rotmod_ltg.parquet \
    --mass-csv /path/to/sparc_mass_table.csv \
    --boundaries-csv /path/to/boundaries.csv \
    --boundary-frac 0.5 \
    --output-dir /Users/leonardspeiser/Projects/GravityCalculator

Notes
- Where to get SPARC masses? Download a SPARC global properties table that contains per-galaxy stellar and gas masses
  (e.g., Mstar_disk, Mstar_bulge, Mgas) from the SPARC Database (CWRU): https://astroweb.cwru.edu/SPARC/
  Then point --mass-csv to that file. If column names differ, rename to the accepted ones above.
...
def main():
    ...
    ap.add_argument('--parquet', type=Path, default=Path('/Users/leonardspeiser/Projects/GravityCalculator/sparc_rotmod_ltg.parquet'))
    ap.add_argument('--mass-csv', type=Path, required=False)
    ap.add_argument('--sparc-mrt', type=Path, required=False,
                    help='Path to SPARC_Lelli2016c.mrt (fixed-width). Used to derive masses and Rd when mass-csv not given.')
    ...
    ap.add_argument('--output-dir', type=Path, default=Path('/Users/leonardspeiser/Projects/GravityCalculator'))
    args = ap.parse_args()
...
def run(args):
    ...
    # Also write boundaries used for transparency
